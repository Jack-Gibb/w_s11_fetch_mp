...
export default function DogForm({ dog, reset, getDogs }) {
  ...
  const postDog = () => {
    ...
    .then(res => {
      if (!res.ok) throw new  Error('Problem POSTing a new dog')
      getDogs()
      navigate('/')
    })
    .catch(err => console.error(err))
  ...
  const putDog = () => {
    ...
    .then(res => {
      if (!res.ok) throw new  Error('Problem PUTing a new dog')
      getDogs()
      reset()
      navigate('/')
    })
    ...
    <div>
      ...
      <form onSubmit={onSubmit}>
        ...
        >
          ...
          {breeds.map(br => <option key={br}>{br}</option>)/* Populate this dropdown using data obtained from the API */}
        </select>
        <label>
          Adopted: <input
            type="checkbox"
